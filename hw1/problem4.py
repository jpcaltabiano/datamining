...
def compute_S(A):
    ...
    sums = A.sum(axis = 0) # sums of all columns
    zero_index = np.where(sums == 0)[0] # indicies of sums[] where value = 0
    for i in zero_index: # for each all 0 col in A, set all elements to 1 instead
        A[:, i] = 1
    S = A / A.sum(axis = 0) # S is trans matrix of reformatted A
