...
def compute_dU(L, U, V, mu=1.):
    ...
    x = np.sum((-2 * np.multiply(L, U)), axis=1, keepdims=True)
    dU = x + ((2 * mu) * U)
...
def compute_dV(L,U, V, mu=1.):
    ...
    x = np.sum((-2 * np.multiply(L, U)), axis=0, keepdims=True)
    dV = x + ((2 * mu) * V)
...
def matrix_decoposition(R, k=5, max_steps=1000000, beta=.01, mu=.01):
    ...
    for _ in range(max_steps): 
        ...
        if np.allclose(U, prev_u): break
        # if np.allclose(np.dot(U,V),R, atol=0.1): break
        ...
        if np.allclose(V, prev_v): break
        # if np.allclose(np.dot(U,V),R, atol=0.1): break

        #########################################

    print("answer: ", np.dot(U,V), '\n\n', R)
    # first test works but not the second?
    return U, V 
