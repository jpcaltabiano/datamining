...
def sum_salaries(T, D):
    ...
    f = filtering(D, 'playerID', T)
    S = sum_column(f, 'salary')
...
def scout():
    ...
    D = pd.read_csv('Batting2001AJS.csv')
    S = sum_salaries(y,D)

    print('Salary: ', 40004167 - S)
    print('Runs: ', 700 - runs(y, D))

    T = ['suzukic01', 'anderbr01', 'nunezab01']
...
def rank_BA(D, min_AB=300,max_salary=1200000):
    ...
    D.sort_values('BA', ascending = False, inplace = True)
...
def rank_OBP(D,min_AB=300,max_salary=1200000):
    ...
    D.sort_values('OBP', ascending = False, inplace = True)
