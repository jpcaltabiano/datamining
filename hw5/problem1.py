...
class TicTacToe(BoardGame):
    ...
    '''
    # ----------------------------------------------
    def get_valid_moves(self, s,x=1):
        '''
           Get a list of available (valid) next moves from a game state of TicTacToe 
            ...
            Hint: in TicTacToe game, you don't need to use the input "x" to solve this problem.
        '''
        #########################################
        ...
        '''
            ...
            Input:
                ...
                x: who's turn in this step of the game, x=1 if it is "X" player's turn. 
                    x=-1 if it's "O" player's turn. 
            Output:
                nx: in the next step of the game, who's turn is it?, nx=1 if it is "X" player's turn, otherwise -1.
            ...
            the game state becomes:
                    s=[[ 0, 1, 1],
                       ...
                       [ 1,-1, 1]]
        '''
        assert self.check_valid_move(s,r,c,x) # check whether the step is valid or not
        s[r,c]=x # fill the empty cell with the current player's stone
        return -x # two players take turns to play
...
class Node:
    '''
        ...
        List of Attributes: 
            s: the current state of the game, an integer matrix  (shape: 3 by 3 in TictacToe, 8 by 8 in Othello). 
                s[i,j] = 0 denotes that the i-th row and j-th column is empty
                s[i,j] = 1 denotes that the i-th row and j-th column is taken by "X". 
                s[i,j] = -1 denotes that the i-th row and j-th column is taken by the "O".
...
class MMNode(Node):
    ...
    '''
    def __init__(self,s,x=1,p=None,c=None,m=None,v=None):
        super(MMNode, self).__init__(s,x,p=p,c=c,m=m,v=v)
    ...
    def expand(self,g):
       ...
       |-------------------------------
               ...
               |    v= None               
               |-----------------------
            Hint: in Othello game, given a parent node p and its child node c,  p.x and c.x NOT necessarily have opposite sign.
            When there is no valid move for one player, that player will give up the move, so in this case the p.x and c.x can be the same.
            You could use the returned value of apply_a_move() to decide who's turn in the next step of the game.
        ...
        # expand the node with one level of children nodes 
            ...
            # create a node for the new game state
